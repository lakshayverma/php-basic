...
?>
    ...
    ?>
                    ...
                    <ul class="nav navbar-nav">
                        ...
                        while (($rec = current($tables)) !== false) :
                            ...
                            <li class=" <?php echo (strcasecmp($table, key($tables)) == 0) ? " current active " : " "; ?>">
                                <a href="?table=<?php echo key($tables); ?>">
                                    <?php echo ucfirst(key($tables)); ?>
                                </a>
                            </li>
                            <?php
                            next($tables);
            ...
            <article id="details" class="panel panel-info">
                ...
                <div class="panel-footer">
                    <ul class="pager">
                        <li class="previous">
                            <a href="?table=<?= $table; ?>&page=<?= $prev_page; ?>&limit=<?= $limit; ?>">Previous</a>
                        </li>
                        <li class="next">
                            <a href="?table=<?= $table; ?>&page=<?= $next_page; ?>&limit=<?= $limit; ?>">Next</a>
                        </li>
                    </ul>
                </div>
            ...
            <?php
            $formFile = "./tableForms/{$table}-form.php";
    ...
    if ($form) :
        ?>
    <script>
        $("#form .btn-group-vertical:last").append(
            "<a class=\"btn btn-default\" href=\"./list_tables.php?table=<?php echo $table; ?>\">Insert a new Record</a>"
        );
        ...
        if ($object->id != '') {
            ...
            ?>
            $("html, body").animate({
                scrollTop: $("#details").parent().offset().top
            }, 500);
            <?php
        }
        ?>
